...
class Rest
{
    ...
    public function readAllUsers($access_token)
    {
        $constants = new Constants($this->clientURL);
        //$user = new User();

        $request = new HTTP_Request2($constants->HOSTNAME . '/learn/api/public/v1/users', HTTP_Request2::METHOD_GET);
        $request->setHeader('Authorization', 'Bearer ' . $access_token);
        $request->setConfig(array(
            'ssl_verify_peer' => $constants->ssl_verify_peer,
            'ssl_verify_host' => $constants->ssl_verify_host
    ...
    public function readAllCourses($access_token)
    {
        $constants = new Constants($this->clientURL);
        //$user = new User();

        $request = new HTTP_Request2($constants->HOSTNAME . '/learn/api/public/v1/courses', HTTP_Request2::METHOD_GET);
        $request->setHeader('Authorization', 'Bearer ' . $access_token);
        $request->setConfig(array(
            'ssl_verify_peer' => $constants->ssl_verify_peer,
            'ssl_verify_host' => $constants->ssl_verify_host
